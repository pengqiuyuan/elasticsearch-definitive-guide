...
看看示例中的数据，感受一下要处理的内容。本数据集有两个索引， `mlmovies` 和 `mlratings` 。首先查看 `mlmovies` ：
...
`offset` 和 `bytes` 。他们是从原始 CSV 文件抽取数据的过程中产生的中间属性。数据集中有 10，681 部影片。
...
只占 0.26% （ `185 / 69796 = 0.00265` ）。因此 _Blades of Glory_ 是一个统计异常：它在喜欢 _Talladega Nights_ 的用户中是非常好的共性。这样就找到了一个好的推荐！
...
这只是 `significant_terms` 它强大的一个示例，一旦开始使用 `significant_terms` ，可能碰到这样的情况，我们不想要最流行的，而想要非常好的共性。这个简单的聚合可以揭示出一些数据里出人意料的复杂趋势。
