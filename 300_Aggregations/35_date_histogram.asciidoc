...
这些很明显都是简单例子，但图表聚合其实是潜力无穷的。((("dashboards", "building from aggregations")))((("Kibana", "dashboard in"))) 如 <<kibana-img>> 展示了 Kibana 中用各种聚合构建的页面。
...
因为聚合的实时性，类似这样的页面很容易查询、操作和交互。这使得它们成为需要分析数据又不会构建 Hadoop 作业的非技术人员的理想工具。

当然，为了构建类似 Kibana 这样的强大页面，你可能需要更深的知识，如范围限定，过滤和排序聚合。
