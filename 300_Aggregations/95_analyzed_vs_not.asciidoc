...
当我们把这些文档加入到 Elasticsearch 中时，字符串 `"New York"` 被分析/分词成 `["new", "york"]` 。这些单独的 tokens ，都被用来填充聚合计数，所以我们最终看到 `new` 的数量而不是 `New York` 。
...
NOTE: 从历史上看，fielddata 是 _所有_ 字段的默认设置。但是 Elasticsearch 已迁移到 doc values 以减少 OOM 的机会。分析的字符串是仍然使用 fielddata 的最后一块阵地。
...
- 这是一个 `not_analyzed` 字段吗？如果是，这将使用内存友好的 doc values 。
